const API_KEY = 'jOJZ14N99X6uG536kvtJU7PydknXVxMA'; // Inserisci qui la tua API Key di Financial Modeling Prep
...
// Funzione per ottenere i dati da Financial Modeling Prep
async function getForexData() {
    ...
    try {
        ...
        console.error("Errore nel recupero dei dati da Financial Modeling Prep:", error);
        return null;
     }
}
...
function calculateSMA(data, timePeriod) {
    if (!data) {
     ...
     const dailyPrices = [data.open, data.previousClose, data.dayHigh, data.dayLow].reverse()
...
function calculateSupportResistance(data) {
    ...
    const dailyPrices = [data.open, data.previousClose, data.dayHigh, data.dayLow, data.price]
...
async function aggiornaSegnaliPagina() {
   ...
   if (!data) {
     ...
     const dailyPrices = [data.open, data.previousClose, data.dayHigh, data.dayLow, data.price].reverse()
